...
const CalculeRates = ({ sessionId, name , categorie }) => {
  ...
  const fetchTopMusics = async () => {
    try {
      ...
      let musicsWithVotes = response.data.musics.filter(
        (music) => music.vote_count > 0
      );

      musicsWithVotes.sort((a, b) => b.vote_count - a.vote_count);

      let maxVotes = musicsWithVotes[0] ? musicsWithVotes[0].vote_count : 0;
      let podiumHeights = musicsWithVotes.map(
        (music) => (music.vote_count / maxVotes) * 300 // 300px est la hauteur maximale
      );

      setTopMusics(
        musicsWithVotes.map((music, index) => ({
          ...music,
  ...
  return (
    <div>
      <h5 className="mb-2 text-2xl font-semibold text-primary-50">{name}</h5>
      <p className="mb-3 text-primary-300">
        <span className="font-medium">Catégorie:</span> {categorie}
      </p>
      ...
      <div className="container podium">
        {topMusics[1] && (
          <div className="podium__item">
            <p className="podium__city">
              {topMusics[1].title} - {topMusics[1].artist}
            </p>
        ...
        {topMusics[0] && (
          <div className="podium__item">
            <p className="podium__city">
              {topMusics[0].title} - {topMusics[0].artist}
            </p>
        ...
        {topMusics[2] && (
          <div className="podium__item">
            <p className="podium__city">
              {topMusics[2].title} - {topMusics[2].artist}
            </p>
